...
/*   Updated: 2022/04/22 17:50:36 by sehhong          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	eats_with_forks(t_philo *philo, int (*check_stat)(t_box *))
{
	...
	print_stat(philo, "\033[33mhas taken a fork\033[0m", FORK1);
	if (check_stat(philo->box) > 0)
		return (-2);
	pthread_mutex_lock(philo->fork2);
	print_stat(philo, "\033[33mhas taken a fork\033[0m", FORK2);
	if (check_stat(philo->box) > 0)
		return (-3);
...
int	sleeps(t_philo *philo, int (*check_stat)(t_box *))
{
	if (check_stat(philo->box) > 0)
		return (-1);
	print_stat(philo, "\033[35mis sleeping\033[0m", SLEEP);
...
int	thinks(t_philo *philo, int (*check_stat)(t_box *))
{
	if (check_stat(philo->box) > 0)
		return (-1);
	print_stat(philo, "\033[34mis thinking\033[0m", THINK);
	return (0);
}
